...
void ConcatenaVetores(int a[], int b[], int r[], int t1, int t2) {
    ...
    for (i=0; i<t1; i++) {
        r[i] = a[i]; // Atribui-se primeiro os elementos do vetor A
    }

    i = 0;

    for (j=0; j<t2; j++) {
        r[t1 + i] = b[j]; // Depois atribui-se os elementos do vetor B
        i++;
    }
}
...
void MostraVetorR(int r[], int t) {
    ...
    for (int i=0; i<t; i++) {
        printf("R[%d] = %d\n", i, r[i]); // Mostra os elementos do vetor concatenado
    }
}
...
int main() {
    ...
    // Leitura dos tamanhos dos vetores
